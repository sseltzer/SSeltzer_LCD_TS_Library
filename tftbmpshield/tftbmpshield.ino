...
void bmpdraw(File f, int x, int y) {
  ...
  for (i=0; i< bmpHeight; i++) {
    ...
    tft.writeRegister(TFTLCD_GRAM_HOR_AD, tx);
    tft.writeRegister(TFTLCD_GRAM_VER_AD, ty);
    tft.writeRegister(TFTLCD_RW_GRAM);
    
/*
    if (tft.getRotation() == 3) {
      tft.writeRegister(TFTLCD_ENTRY_MOD, 0x1028);
      tft.writeRegister(TFTLCD_GRAM_HOR_AD, x+i);
      tft.writeRegister(TFTLCD_GRAM_VER_AD, y);
      tft.writeRegister(TFTLCD_RW_GRAM);
    } else if  (tft.getRotation() == 2) {
      tft.writeRegister(TFTLCD_ENTRY_MOD, 0x1020);
      tft.writeRegister(TFTLCD_GRAM_HOR_AD, x+bmpWidth);
      tft.writeRegister(TFTLCD_GRAM_VER_AD, y+i);
      tft.writeRegister(TFTLCD_RW_GRAM);
    } else if  (tft.getRotation() == 1) {
      tft.writeRegister(TFTLCD_ENTRY_MOD, 0x1018);
      tft.writeRegister(TFTLCD_GRAM_HOR_AD, x+bmpHeight-1-i);
      tft.writeRegister(TFTLCD_GRAM_VER_AD, y);
      tft.writeRegister(TFTLCD_RW_GRAM);
    } else if  (tft.getRotation() == 0) {
      tft.writeRegister(TFTLCD_ENTRY_MOD, 0x1030);
      tft.writeRegister(TFTLCD_GRAM_HOR_AD, x);
      tft.writeRegister(TFTLCD_GRAM_VER_AD, y+bmpHeight-i);
      tft.writeRegister(TFTLCD_RW_GRAM);
    }*/
    ...
    for (j=0; j<bmpWidth; j++) {
      ...
      // lol grayscale?
      //p = (((((sdbuffer[buffidx++] >> 3) << 6) | (sdbuffer[buffidx++] >> 2)) << 5) | (sdbuffer[buffidx++] >> 3));
